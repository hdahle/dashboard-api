...
function main() {
  ...
  if (redisKey === undefined || redisKey === true || redisKey === '') {
    console.log('Usage: node covid.js --key <rediskeyprefix>');
    console.log('The Redis keys will be "<rediskeyprefix>-deaths" and "<rediskeyprefix>"-confirmed')
    return;
  }
  ...
  redClient.on('ready', function () {
    ...
    //urls.forEach(url => processFile(url.url, redisKey + '-' + url.key, redClient));
    processFile(urls[0].url, redisKey + '-' + urls[0].key);
  });
...
function processFile(url, key) {
  console.log('processFile:', url, key)
  let countries = [];
  let csvDates = [];
  ...
  function processCsvLine(csv) {
    console.log('processCsvLine');
  ...
  function processCsvEnd() {
    console.log('processCsvEnd:', countries.length)
    ...
    redClient.set(key, val, function (error, result) {
      ...
      setTimeout(() => { process.exit(); }, 10000); // We are done
    });
  }
}
