...
function parseArgv(argv) {
  ...
  let countriesOK = cList.every(x => ['World', 'All', 'top20'].includes(x) || !(p(x, null) === 1 && p(null, x).length === 0));
  ...
  if (!countriesOK || fn === undefined || fn === true || !fs.existsSync(fn) || redisKey === undefined || redisKey === true || redisKey === '') {
    ...
    console.log('\t--countries=regions\n\t\tgenerate time-series data for Europe, Asia, North America, Central America, Africa, Oceania')
...
function processFile(fn, redisKey, redClient, cList) {
  ...
  fs.createReadStream(fn)
    ...
    .on('end', () => {
      ...
      ['World', 'Europe', 'Asia', 'Africa', 'Oceania', 'North America', 'Central America', 'South America'].forEach(reg => {
        ...
        allCountries.forEach(c => {
          if (cList.includes(c.country)) {
            let tmp = c.data.map(x => ({
              t: x.t,
              d: x.d,
              y: x.y > 100 ? x.c : null // Set c (change) to 'null' if number of cases (y) is less than 100
            }));
            val.data.push({
              country: c.country,
