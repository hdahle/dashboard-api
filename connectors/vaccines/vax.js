...
let partial = argv.partial;
let full = argv.full;
let doses = argv.doses;
...
let countriesOK = cList.every(x => ['All'].includes(x) || worldPop.findIndex(c => c.c === x) > -1);
if (!countriesOK || fn === undefined || fn === true || !fs.existsSync(fn) || redisKey === true || redisKey === '') {
  console.error('Usage:\n\tnode vaccines.js --file <csvfile> [--countries <country,country,...>] [--key <rediskey>] [--partial] [--full] [--doses]\n');
...
function processFile(fn, cList) {
  ...
  fs.createReadStream(fn)
    ...
    .on('data', csv => {
      ...
      // Dates go into the labels array
