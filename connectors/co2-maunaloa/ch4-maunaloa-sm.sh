...
awk -v d="${DATE}" 'BEGIN {ORS="";
            ...
            print "\"info\":\"Globally averaged monthly mean CH4 abundance measured from marine surface sites. Numbers are dry air mole fraction in ppb. The original dataset has _average_ and _trend_ values for each month, this dataset uses _average_\", "
            print "\"accessed\":\"" d "\", "
            print "\"data\": ["
            NOTFIRST=0
           }
     /^#/  {next}
     NF==7 {if (NOTFIRST) print ", "
            NOTFIRST=1
