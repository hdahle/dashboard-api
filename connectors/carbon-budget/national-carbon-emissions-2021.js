...
(() => {
  ...
  let result = {
    ...
    link: argv.link || 'https://www.globalcarbonproject.org',
    data: {
      yAxisLabel: 'Gt',
      datasets: []
    }
  ...
  if (fn === undefined || fn === true || fn === '' || !fs.existsSync(fn)) {
    ...
    console.log('Usage: node script --file <filename> [--countries <countries>] [--key <rediskey>] --source <source> --link <hyperlink to source>');
  ...
  fs.createReadStream(fn)
    ...
    .on('data', csvRow => {
      if (csvRow[1].includes('Source of')) {
        ...
        for (let i = 0; i < nCols; i++) {
          ...
          result.data.datasets[i].data.push({
            x: parseInt(year, 10), // avoid quotemarks around year
            y: Math.floor(y * 366.4) / 100000
          });
        }
      }
    })
    .on('end', () => {
      ...
      if (cList.length) {
        result.data.datasets = result.data.datasets.filter(x => cList.includes(x.label.toLowerCase()));
      }

      result.data.datasets.forEach(x => {
      ...
      if (key !== undefined && key !== true && key !== '') {
        redClient.set(key, json, function (error, result) {
          if (result) {
            console.log('Wrote', json.length, 'bytes to Redis key: ' + key);
          }
          else {
